...
description: Implement and verify the AutoTestTools animation slice for the Unreal Editor plugin at D:\MCP\Plugins\AutoTestTools when Phase 1 PIE/session helpers and the input slice already exist. Use it to add UAutoTestAnimationUtils plus Python AnimationHelper wrappers, then run Python syntax checks, UBT compilation, and an UnrealEditor runtime smoke test that validates montage play/active/stop behavior inside PIE.
...
## When to use
- 用户要在 `D:\MCP\Plugins\AutoTestTools` 现有 Phase 1 + input 切片基线上继续落地 animation 切片。
...
## Steps
1. 先读取现有基线文件：
   - `D:\MCP\Plugins\AutoTestTools\Source\AutoTestTools\Public\AutoTestActorUtils.h`
   - `D:\MCP\Plugins\AutoTestTools\Source\AutoTestTools\Public\AutoTestInputUtils.h`
   - `D:\MCP\Plugins\AutoTestTools\Content\Python\auto_test_tools\actor.py`
   - `D:\MCP\Plugins\AutoTestTools\Content\Python\auto_test_tools\input.py`
   - `D:\MCP\Plugins\AutoTestTools\Content\Python\auto_test_tools\__init__.py`
2. 新增 C++ 动画桥接：
...
6. 新增 Python 语义层：`D:\MCP\Plugins\AutoTestTools\Content\Python\auto_test_tools\animation.py`，封装 `AnimationHelper`，至少提供：
   - `get_skeletal_mesh()`
...
11. 用 UBT 直接编译编辑器目标：
   - `"/c/Program Files/dotnet/dotnet.exe" "D:/UnrealEngine/Engine/Binaries/DotNET/UnrealBuildTool/UnrealBuildTool.dll" MCPEditor Win64 Development -Project="D:/MCP/MCP.uproject" -NoHotReloadFromIDE -WaitMutex`
12. 在工作区写一个 runtime runner，例如 `C:\Users\Administrator\WorkBuddy\Claw\autotesttools_animation_runtime_regression.py`：
   - 将 `D:\MCP\Plugins\AutoTestTools\Content\Python` 加入 `sys.path`
...
13. 用 Unreal Editor 执行 runtime 回归：
   - `"D:/UnrealEngine/Engine/Binaries/Win64/UnrealEditor.exe" "D:/MCP/MCP.uproject" -unattended -stdout -FullStdOutLogOutput -ExecutePythonScript="C:/Users/Administrator/WorkBuddy/Claw/autotesttools_animation_runtime_regression.py"`
